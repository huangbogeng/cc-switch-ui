...
pub fn transform(input: TransformInput) -> Result<TransformOutput, ProviderError> {
    // TODO: get codex_fast_mode from provider config
    let codex_fast_mode = false;

    let transformed =
        crate::proxy::transform_responses::anthropic_to_codex_responses(
            input.body,
            input.prompt_cache_key.as_deref(),
            codex_fast_mode,
        )
        .map_err(|e| ProviderError::TransformFailed(e))?;
